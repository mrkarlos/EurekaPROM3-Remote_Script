...
from ableton.v2.base import NamedTuple, depends, infinite_context_manager, is_contextmanager, is_iterable, lazy_attribute, listenable_property, listens, old_hasattr, task
...
from ableton.v2.control_surface.layer import CompoundLayer, Layer
from ableton.v2.control_surface.resource import StackingResource
...
def tomode(thing):
    logger.info('tomode()')
    if thing is None:
        logger.info('tomode() - thing is None')
        return Mode()
    if isinstance(thing, Mode):
        logger.info('tomode() - thing isintance of Mode')
        return thing
    if old_hasattr(thing, 'set_enabled'):
        logger.info('tomode() - old_hasattr set_enabled')
        return EnablingMode(thing)
    if isinstance(thing, tuple):
        logger.info('tomode() - thing isintance of tuple')
        if len(thing) == 2:
            logger.info('tomode() - tuple len == 2')
            if isinstance(thing[0], Component):
                logger.info('tomode() - thing[0] isinstance of Component')
                if isinstance(thing[1], (Layer, CompoundLayer)):
                    logger.info('tomode() - thing[1] isinstance of CompoundLayer')
                    return LayerMode(*thing)
            if callable(thing[0]):
                logger.info('tomode() - thing[0] is callable')
                if callable(thing[1]):
                    logger.info('tomode() - thing[1] is callable')
                    mode = Mode()
                    mode.enter_mode, mode.leave_mode = thing
                    return mode
    if is_iterable(thing):
        logger.info('tomode() - is_iterable(thing)')
        return CompoundMode(*thing)
    if is_contextmanager(thing):
        logger.info('tomode() - is_contextmanager(thing)')
        return ContextManagerMode(thing)
    if callable(thing):
        logger.info('tomode() - callable(thing)')
        mode = Mode()
...
class LayerMode(LayerModeBase):
    ...
    def enter_mode(self):
        logger.info('LayerMode: enter_mode()')
        self._get_component().layer = self._layer

    def leave_mode(self):
        logger.info('LayerMode: leave_mode()')
...
class AddLayerMode(LayerModeBase):
    ...
    def enter_mode(self):
        logger.info('AddLayerMode: leave_mode()')
...
class CompoundMode(Mode):
    ...
    def enter_mode(self):
        logger.info('enter_mode()')
        for mode in self._modes:
            mode.enter_mode()

    def leave_mode(self):
        logger.info('leave_mode()')
...
class ModesComponent(Component):
    ...
    def __init__(self, enable_skinning=False, support_momentary_mode_cycling=True, *a, **k):
        logger.info('ModesComponent: __init__()')
        ...
        if self.is_enabled():
            if self.selected_mode != mode:
                if mode is not None:
                    ...
                    logger.info('selected_mode() - mode was None, release_all()')
                    self._mode_stack.release_all()
        else:
    ...
    def pop_unselected_modes(self):
        logger.info('pop_unselected_modes()')
    ...
    def _get_mode_behaviour(self, name):
        logger.info('_get_mode_behaviour()')
        entry = self._mode_map.get(name, None)
    ...
    def _update_mode_buttons(self, selected):
        logger.info('_update_mode_buttons()')
        if self.is_enabled():
    ...
    def _do_enter_mode(self, name):
        logger.info('ModesComponent: _do_enter_mode()')
    ...
    def _do_leave_mode(self, name):
        logger.info('ModesComponent: _do_leave_mode()')
